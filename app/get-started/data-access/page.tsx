    ...
    <div className="min-h-screen bg-white p-4 text-black">
      ...
      <div className="bg-[#e6eef9] p-4 mb-4">
        <div className="flex flex-wrap gap-8">
          <div>
            <label className="text-black font-medium block mb-2">Datasource:</label>
            <select 
              value={datasource}
              onChange={(e) => setDatasource(e.target.value)}
              className="border rounded px-3 py-1 min-w-[200px] text-black"
            >
              <option value="INSAT-3DR">INSAT-3DR</option>
            </select>
          </div>
          <div>
            <label className="text-black font-medium block mb-2">Category:</label>
            <select 
              value={category}
        ...
        <table className="min-w-full border">
          <thead>
            <tr className="bg-blue-700">
              ...
              <th className="border px-4 py-2 text-black">Product Description</th>
              <th className="border px-4 py-2 text-black">Processing Level</th>
              <th className="border px-4 py-2 text-black">Start Date</th>
              <th className="border px-4 py-2 text-black">End Date</th>
              <th className="border px-4 py-2 text-black">Temporal Resolution</th>
              <th className="border px-4 py-2 text-black">Order Data</th>
            </tr>
          </thead>
          <tbody>
            {productData.map((product) => (
              <tr key={product.srNo} className="hover:bg-gray-50">
                <td className="border px-4 py-2 text-black">{product.srNo}</td>
                ...
                <td className="border px-4 py-2 text-black">{product.processingLevel}</td>
                <td className="border px-4 py-2 text-black">{product.startDate}</td>
                <td className="border px-4 py-2 text-black">{product.endDate}</td>
                <td className="border px-4 py-2 text-black">{product.temporalResolution}</td>
                <td className="border px-4 py-2">
                  <button className="bg-blue-500 text-black p-2 rounded-full hover:bg-blue-600">
